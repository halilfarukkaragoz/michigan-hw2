...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = X.mm(W1) + b1
    h_relu = hidden.clamp(min = 0)
    scores = h_relu.mm(h_relu) + b2
...
def nn_forward_backward(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    grad_y_pred = 2.0 * (scores-y)
    grads["W2"] = h1.clamp(min = 0).t().mm(grad_y_pred) # 
    grad_h_relu = grad_y_pred.mm(W2.t())
    grad_h = grad_h_relu.clone()
    grad_h[h1 < 0] = 0
    grads["W1"] = X.t().mm(grad_h)
...
def nn_predict(
    ...
    """
    y_pred = None
    scores , _ = nn_forward_pass(Dict,X)
    ###########################################################################
